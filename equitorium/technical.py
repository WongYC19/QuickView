...
class Indicators:
    ...
    def rsi(self, period=14, lower=30, upper=70, show_signal=True):
        ...
        avg_gain = (
            (period - 1) * avg_gain["prev_gain"].shift(1) + avg_gain["close"]
        ) / period
        avg_loss = (
            (period - 1) * avg_loss["prev_loss"].shift(1) + avg_loss["close"]
        ) / period
    ...
    def ma_crossover(
        self, period1=50, period2=20, ema=False, show_signal=True, kind="close"
    ):
        ...
        if show_signal:
            output[f"ma({period1}-{period2})"] = ma1 - ma2
        return output
    ...
    def obv_crossover(self, period1=20, period2=50, show_signal=True):
        ...
        if show_signal:
            output[f"obv({period1}-{period2})"] = obv1 - obv2
        return output
    ...
    def macd(self, period1=12, period2=26, kind="close", show_signal=True):
        ...
        output = pd.concat((ema1, ema2), 1)
        output.columns = [f"macd({period1})", f"macd({period2})"]

        if show_signal:
            output[f"macd({period1}-{period2})"] = ema1 - ema2
        return output
...
class Caster:
    def __init__(self, price_path=None, n_splits=3):
        ...
        self.prices.code = self.prices.code.replace("[A-Za-z]+", "", regex=True).astype(
            int
        )
        self.prices = (
            self.prices.drop_duplicates(subset=["date", "code"], keep="last")
        ...
        self.data = {
            code: (
                self.prices.iloc[loc["start"] : loc["end"], :]
                .drop("code", 1)
                .set_index("date")
            )
            for code, loc in location.items()
        }
        ...
        self.meta = os.path.join(
            f"C:/Users/{getuser()}", "Documents", "bursa", "metadata.xlsx"
        )
        assert os.path.isfile(self.meta)
        metadata = pd.read_excel(self.meta)
        metadata = metadata.dropna(subset=["bursacode"], axis=0).applymap(
            lambda x: x.replace("amp;", "") if type(x) == str else x
        )
        ...
        self.sectors = (
            metadata.set_index("bursacode")["industrygroupcode"].dropna().to_dict()
        )
    ...
    def get_price(self, code, max_points=None):
        ...
        try:
            ...
            warn(
                f"{code} not found. Please check available stock codes by looking at Price().codes"
            )
        else:
            ...
            if (
                max_points is not None and max_points > price.shape[0]
    ...
    def show_chart(self, price):
        ...
        vol_colors = np.where(
            price.close.diff(1) > 0, INCREASING_COLOR, DECREASING_COLOR
        )
        fig = go.Figure(
            make_subplots(
                shared_xaxes=True,
        ...
        fig.update_layout(
            title=dict(text=self.title, x=0.5, y=0.97, xanchor="center", yanchor="top"),
        ...
        fig.update_layout(
            ...
            yaxis1=dict(domain=[0.25, 0.97]),
            yaxis2=dict(domain=[0, 0.25]),
        )

        fig.update_xaxes(
            showgrid=True, gridwidth=2, gridcolor="rgb(234,234,234)", row=1
        )
        fig.update_yaxes(
            showgrid=True, gridwidth=1, gridcolor="rgb(234,234,234)", row=1
        )
        return fig
    ...
    def plot_prediction(self, y, yhat):
        ...
        plt.title(
            "Mean Square Error: %.2f \nMean Absolute Error: %.2f" % (mse, mae),
            fontsize=12,
        )
    ...
    def forecast(self, code, n_steps=3, train_size=0.8, max_size=None, plot=True):
        ...
        if plot:
            self.plot_prediction(y, valid_pred)
